...
- **已归档标识**: 使用 "已归档" 文字标签，灰色背景
- **删除线**: 已归档项目名称添加删除线 (line-through)
- **透明度**: 适当降低已归档项目的颜色透明度 (opacity-50)
- **背景区分**: 已归档项目使用较浅的背景色 (bg-gray-100/50)
- **操作按钮**: 归档按钮使用琥珀色 (amber)，取消归档使用绿色 (green)
- **折叠展示**: 已归档项目默认折叠，点击展开查看

### 5.7 固定布局设计原则

- **Flex 布局**: 使用 flex-none 固定头部和底部，flex-1 让中间区域自适应
- **视口高度**: 外层容器使用 h-screen 占满视口
- **独立滚动**: 中间内容区域使用 overflow-y-auto 独立滚动
- **层级管理**: 固定元素使用 z-20 确保在最上层
- **阴影分隔**: 底部固定栏使用阴影与内容区分 (shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)])
