...
- **背景色**: bg-gray-50/50，与内容区域区分
- **图标**: 分类使用 Folder 图标，主题使用 Palette 图标
- **颜色标识**: 分类/主题的颜色以圆角标签形式展示
- **任务计数**: 显示组内任务数量，使用灰色背景小标签
- **折叠指示**: 使用 ChevronRight 图标，旋转动画指示展开状态

#### 分组内容设计

- **内边距**: p-2，紧凑但留有呼吸空间
- **任务卡片**: 使用紧凑模式 (compact)，减少内边距
- **动画效果**: 使用 height + opacity 动画实现平滑折叠/展开
- **过渡时间**: duration-200，快速响应用户操作
